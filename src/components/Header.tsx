      ...
      <div className="container flex h-16 max-w-screen-2xl items-center">
        ...
        <div className="hidden md:flex flex-1 items-center justify-between">
          <nav className="flex items-center space-x-6 text-sm font-medium">
            <Link to="/" className={`transition-colors hover:text-foreground/80 ${isActive('/') || isActive('/home') ? 'text-foreground font-bold' : 'text-foreground/60'}`}>
              Home
            </Link>
        ...
        <div className="md:hidden flex flex-1 items-center justify-end">
          <Button variant="ghost" size="sm" className="px-0 text-base hover:bg-transparent focus-visible:bg-transparent focus-visible:ring-0 focus-visible:ring-offset-0" onClick={() => setIsMenuOpen(!isMenuOpen)}>
        ...
        {isMenuOpen && <div className="fixed inset-0 top-16 z-50 grid h-[calc(100vh-4rem)] grid-flow-row auto-rows-max overflow-auto p-6 pb-32 shadow-md animate-in md:hidden bg-background">
            <div className="relative z-20 grid gap-6 rounded-md bg-background p-4">
              ...
              <nav className="grid grid-flow-row auto-rows-max text-sm">
                <Link to="/" className={`flex w-full items-center rounded-md p-2 text-sm font-medium ${isActive('/') || isActive('/home') ? 'bg-muted' : ''}`} onClick={() => setIsMenuOpen(false)}>
                  Home
                </Link>
